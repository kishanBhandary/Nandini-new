...
const REMEMBERED_WORKER_KEY = 'nandini:remembered-worker-username';
...
export default function WorkerLoginScreenPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/login', {
        ...
        body: JSON.stringify({ username: trimmedUsername, password, role: 'WORKER' }),
      });
      ...
      if (rememberUsername) {
        window.localStorage.setItem(REMEMBERED_WORKER_KEY, trimmedUsername);
      } else {
  ...
  return (
    <main className="auth-shell admin-login-page">
      ...
      <section className="auth-panel admin-login-panel" aria-label="Worker login form">
        ...
        <form className="auth-form screenshot-style" onSubmit={handleSubmit}>
          ...
          <div className="auth-row admin-login-controls-row">
            <label className="auth-remember" htmlFor="remember-worker-username">
              <input
                id="remember-worker-username"
                type="checkbox"
